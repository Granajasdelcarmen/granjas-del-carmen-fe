...
import { useSellSheep } from 'src/hooks/useSheep';
import { Sheep } from 'src/types/api';

interface SheepSaleModalProps {
  sheep: Sheep | null;
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
...
export function SheepSaleModal({ 
  sheep, 
  isOpen, 
  onClose, 
  onSuccess,
  ...
  const sellMutation = useSellSheep();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.message || 'Error al vender la oveja');
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Vender Oveja">
      <div className="space-y-4">
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <p className="text-sm text-green-800">
            <strong>💰 Venta:</strong> Estás a punto de vender la oveja <strong>{sheep.name}</strong>.
