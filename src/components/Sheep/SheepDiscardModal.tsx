...
export function SheepDiscardModal({ 
  sheep, 
  isOpen, 
  onClose, 
  onSuccess 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.message || 'Error al descartar la oveja');
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Descartar Oveja">
      <div className="space-y-4">
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <p className="text-sm text-yellow-800">
            <strong>⚠️ Atención:</strong> Estás a punto de marcar la oveja <strong>{sheep.name}</strong> como descartada.
