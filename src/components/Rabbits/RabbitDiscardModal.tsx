...
export function RabbitDiscardModal({ 
  rabbit, 
  isOpen, 
  onClose, 
  onSuccess 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.message || 'Error al descartar el conejo');
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Descartar Conejo">
      <div className="space-y-4">
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <p className="text-sm text-yellow-800">
            <strong>⚠️ Atención:</strong> Estás a punto de marcar el conejo <strong>{rabbit.name}</strong> como descartado.
